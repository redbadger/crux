...
fn the_swift_source_compiles() {
    ...
    let shipped = fs::read_to_string(dir.path().join("App/Sources/App/KeyValue.swift"))
        .expect("should write the handler beside the module");
    assert!(shipped.contains("public protocol KeyValueHandler: Sendable {"));
    assert!(shipped.contains("public final class UserDefaultsKeyValueHandler"));

    swift_build(&dir.path().join("App"));
}

/// Kotlin has no compiler this test can count on, and no example shell
/// registers `KEY_VALUE` — the weather shells keep their own platform stores —
/// so `KeyValue.kt` is held to review and to this check that it lands where the
/// app will find it, under the module's package.
#[test]
fn the_kotlin_source_is_emitted() {
    ...
    let shipped = fs::read_to_string(dir.path().join("com/example/shared/KeyValue.kt"))
        .expect("should write the handler beside the module");
    assert!(shipped.starts_with("package com.example.shared\n"));
    assert!(shipped.contains("interface KeyValueHandler {"));
    assert!(shipped.contains("class FileKeyValueHandler("));
}
...
fn the_typescript_source_compiles() {
    ...
    assert!(declarations.contains("localStorageKeyValueHandler"));
}
...
fn the_csharp_source_compiles() {
    ...
    let shipped = fs::read_to_string(dir.path().join("Example/Shared/KeyValue.cs"))
        .expect("should write the handler beside the module");
    assert!(shipped.contains("public interface IKeyValueHandler"));
...
// emitted beside. Both of the bugs that reached a user got through that:
// `File.createTempFile("kv", ..)` threw on every write, because a prefix has
// to be at least three characters, and the temporary file it made sat in the
// store directory where `listKeys` reported it as a key. Neither is a compile
