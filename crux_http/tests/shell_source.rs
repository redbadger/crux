...
//! `crux_http::HTTP` against a small app and hands the generated package to
...
fn the_swift_source_compiles() {
    ...
    let shipped = fs::read_to_string(dir.path().join("App/Sources/App/Http.swift"))
        .expect("should write the handler beside the module");
    assert!(shipped.contains("public protocol HttpHandler: Sendable {"));
    assert!(shipped.contains("public final class URLSessionHttpHandler: HttpHandler {"));
...
fn the_kotlin_source_is_emitted() {
    ...
    let shipped = fs::read_to_string(dir.path().join("com/example/shared/Http.kt"))
        .expect("should write the handler beside the module");
    assert!(shipped.starts_with("package com.example.shared\n"));
    assert!(shipped.contains("interface HttpHandler {"));
    assert!(shipped.contains("class UrlConnectionHttpHandler("));
}
...
fn the_typescript_source_compiles() {
    ...
    assert!(declarations.contains("fetchHttpHandler"));
}
...
fn the_csharp_source_compiles() {
    ...
    let shipped = fs::read_to_string(dir.path().join("Example/Shared/Http.cs"))
        .expect("should write the handler beside the module");
