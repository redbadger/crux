...
mod tests {
    ...
    fn resolve_with_the_wrong_effect_variant() {
        ...
        assert_eq!(
            error.to_string(),
            format!(
                "could not process response: Request {} expects `Http` (variant 0), but response id {:#010x} carries `Render` (variant 1).",
                http.id.sequence(),
                mangled.0
            )
        );
    ...
    fn resolve_with_the_wrong_kind_bit() {
        ...
        assert_eq!(
            error.to_string(),
            format!(
                "could not process response: Request {} expects the Request kind, but response id {:#010x} carries the Stream kind.",
                http.id.sequence(),
                mangled.0
            )
        );
