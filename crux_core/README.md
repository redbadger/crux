...
generated by [BoltFFI](https://www.boltffi.dev/), for Apple platforms, Android,
WebAssembly and Windows alike.

In order to both send more complex data than the FFI layer supports, and enforce
the message passing semantics, all messages are serialized (with bincode) as
they cross the boundary. The types on the other side — `Event`, `ViewModel`
and the effect payloads — are generated for Swift, Kotlin, TypeScript and C#
from the Rust definitions, using [facet](https://docs.rs/facet) reflection.
